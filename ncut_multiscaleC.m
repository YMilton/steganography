function [W,C] = ncut_multiscaleC(image,options)
...
t= cputime;
[layersp,layersq,layersindexes1,layersindexes2,layersindexes3,layersindexes4,layerweight,layersscales,layersradius,layerslocation1,layerslocation2,layerslocation3,layerslocation4,C]=compute_layers_C_multiscale(p,q);
[edgeVariance,sigmaI] = computeParametersW(image);
W=computeMultiscaleW(image,layers,edgeVariance,sigmaI,options);
...
function [layersp,layersq,layersindexes1,layersindexes2,layersindexes3,layersindexes4,layerweight,layersscales,layersradius,layerslocation1,layerslocation2,layerslocation3,layerslocation4,C]=compute_layers_C_multiscale(p,q)
...
layersp=[512 171 57 19];
layersq=[512 171 57 19];
layersindexes1=[1:262144]' ;
layersindexes2=[262145:291385]';
layersindexes3=[291386:294634];
layersindexes4=[294635:294995];
layerweight=[3 4 1 2];
layersscales=[1 2 3 3];
layersradius=[2 3 4 6];

[C,C12]=computeMultiscaleConstraints(layers);
p1=p(1);
q1=q(1);
layerslocation1=1:p(1)*q(1);
...
for i=2:4
    ...
    location(i)=sub2ind2([p,q],xi,yi);
end
layerslocation2=location(2);
layerslocation3=location(3);
layerslocation4=location(4);
...
function [edgeVariance,sigmaI] = computeParametersW(image)
...
function W = computeMultiscaleW(image,layers,edgeVariance,sigmaI,options)
...
for i=1:ns
    ...
    Ws{i}=computeScale(image,layers(i),edgeVariance,sigmaI,optionsi);
end
[p,q,~]=size(image);
